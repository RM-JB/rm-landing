...
return String(str).replace(/[&<>"']/g, char => ({
  '&': '&amp;',
  '<': '&lt;' , '>' : '&gt;' , '"' : '&quot;' , "'" : '&#039;' }[char])); } function decodeHTML(str) { const
    textarea=document.createElement('textarea'); textarea.innerHTML=str; return textarea.value; } function
    formatHTML(html) { const tab='  ' ; const template=document.createElement('template');
    template.innerHTML=html.trim(); const inlineTags=new Set([ 'a' , 'span' , 'strong' , 'em' , 'b' , 'i' , 'small'
    , 'button' , 'h1' , 'h2' , 'h3' , 'h4' , 'h5' , 'h6' , 'p' ]); const voidTags=new Set([ 'img' , 'br' , 'hr'
    , 'input' , 'meta' , 'link' , 'area' , 'base' , 'col' , 'embed' , 'param' , 'source' , 'track' , 'wbr' ]); function
    attrsToString(el) { return [...el.attributes] .map(attr=> `${attr.name}="${attr.value.replace(/"/g, '&quot;')}"`)
    .join(' ');
    }
    ...
    return attrs ? `<${el.tagName.toLowerCase()} ${attrs}>` : `<${el.tagName.toLowerCase()}>`;
        ...
        // Remove parallax transform variable from inline styles
        all.forEach(node => {
